...
export default function AvailableFlights() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const faresNumeric = parseInt(
        initialState.price.replace(/[^0-9]/g, ''),
        10
      );
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span
                        className={`ml-auto text-xs font-medium px-2 py-1 rounded-full ${statusClass(item.status)}`}
                      >
                        {item.status}
                      </span>
                  ...
                  {item.schedule_id ? (
                    ...
                    <Link
                      to={
                        id > 0
                          ? `/invoice/${item.schedule_id}-${id}?fare=${item.fares}`
                          : '/customer-signin'
                      }
                    ...
                    <span className="flex-shrink-0 px-4 py-2 rounded-xl text-sm font-semibold bg-gray-100 text-gray-400 cursor-not-allowed">
                      Unavailable
                    </span>
