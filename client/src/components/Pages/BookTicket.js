...
export default function BookTicket() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      Swal.fire('Search failed', 'Please check your search criteria and try again.', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="pt-24 pb-16 px-4 max-w-xl mx-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
          {/* Departure */}
          <div>
            <label htmlFor="departure" className="block text-sm font-medium text-gray-700 mb-1.5">Flying From</label>
            <div className="relative">
              <FaPlaneDeparture className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-400" size={16} />
              <select id="departure" {...register('departure', { required: 'Departure is required' })}
          ...
          {/* Arrival */}
          <div>
            <label htmlFor="arrival" className="block text-sm font-medium text-gray-700 mb-1.5">Flying To</label>
            <div className="relative">
              <FaPlaneArrival className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-400" size={16} />
              <select id="arrival" {...register('arrival', { required: 'Arrival is required' })}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="departureDate" className="block text-sm font-medium text-gray-700 mb-1.5">Departure Date</label>
              <input id="departureDate" type="date" {...register('departureDate', { required: 'Required' })}
            ...
            <div>
              <label htmlFor="returnDate" className="block text-sm font-medium text-gray-700 mb-1.5">Return Date</label>
              <input id="returnDate" type="date" {...register('returnDate', { required: 'Required' })}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="class" className="block text-sm font-medium text-gray-700 mb-1.5">Class</label>
            ...
            <div>
              <label htmlFor="price" className="block text-sm font-medium text-gray-700 mb-1.5">Max Price</label>
              <select id="price" {...register('price', { required: true })}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-sky-700 hover:bg-sky-800 text-white font-semibold py-3 rounded-xl text-sm transition-colors disabled:opacity-60">
