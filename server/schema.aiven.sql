-- ============================================================
-- AirOpsManager — Aiven (cloud) schema
-- Differences from schema.sql (local):
--   1. BOOKING PK is ticket_id only (client_id nullable; trigger inserts with NULL client_id)
--   2. FlightBooking has client_id column (used by search.js queries)
--   3. Views use exact table-name casing (Linux MySQL is case-sensitive)
--   4. Trigger uses DELIMITER syntax required by MySQL Workbench
-- ============================================================
...
-- ticket_id is PK (not composite) so trigger can insert with client_id=NULL
CREATE TABLE IF NOT EXISTS BOOKING (
  ...
  flight_no    INT,
  fares        FLOAT,
  CONSTRAINT PK_BOOKING       PRIMARY KEY (ticket_id),
...
-- client_id required by search.js queries
...
-- Views (exact casing matches table names — required on Linux/Aiven)
